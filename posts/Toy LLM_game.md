...
这篇文章主要探讨了LLM（大语言模型）在小游戏设计中的应用机会与方向，并结合模拟经营、冒险探索、肉鸽等类型分析了适合切入的玩法领域，核心要点如下：

### 1. 现状与机会

传统高深度玩法虽受好评，但对轻度玩家吸引力有限。

LLM暂不擅长深度玩法，但有潜力在玩法广度、视听交互、叙事玩法上做出特色。

小型LLM游戏易测试、好统筹，是实验新技术的理想场景。

### 2. 潜在适配类型

模拟经营：轻度玩法多、探索感强，适合LLM生成丰富交互与轻量叙事，但需保持一致的视听风格。

冒险探索：可借助LLM生成无限的探索场景与地形设计，需要可调度的美术资源与足够自由度。

肉鸽（Roguelike）：核心不一定是高难度，而是多样的build体验；LLM生成内容需控制随机性，以免增加过高决策成本。

### 3. 三大核心生成方向

叙事玩法：围绕人物生成、采用单元剧/碎片化叙事以减少一致性压力。

视听交互玩法：区别于传统PCG，LLM可生成新颖有趣的交互元素和地形，但需有可拓展的资源库与创意技巧。

轻度玩法：可在既有核心玩法上增加新流派/小玩法，需理解何为“有趣”的设计并能调度相关资源。

### 4. 设计技巧与实现路径

固定生成范围与内容形式（如二维像素优于三维建模）。

为LLM设计可调度的“游戏语言”，类似UGC平台的接口。

优化LLM调用和生成的适配能力，通过用户反馈提升趣味性。

### 5. 一句话方案

构建或利用支持UGC的游戏生成器，让LLM调用生成趣味性内容（如奇观、肉鸽流派），维持简单核心玩法，并通过玩家反馈不断优化生成质量。
...
* 如果LLM只生成文字，那么游戏主体将只是文字冒险游戏，这是比较小众的。
...
2. 这对LLM systems的要求体现在：能够调度相关资源进行生成，且能够进行具有趣味性的生成。

> 如何设计LLM以实现上述需求，可以构成研究问题。

3. 可以使用一些技巧：

* 维持核心玩法的同时，限定LLM生成的内容形式。

> 固定生成的内容要求，尤其是更低的内容要求 (例如，二维像素相比三维建模)，有助于LLM更好的控制。对限定的叙事、视听交互、玩法来说，趣味性生成也可能更容易被捕捉和描述，从而供LLM理解和生成。

* 设计供LLM调度的‘语言’。就像I Wanna Maker、Minecraft一样，一些具有可拓展性的支持用户生成内容 (User generated Content, UGC)的游戏设计，同样可能适合LLM用于生成游戏内容。

> 需要协调和整合来自多方面的经验。

* 优化LLM调度资源进行生成的能力。人类需要通过学习掌握mod开发，LLM也需要做相应的适配。

## 一句话方案

1. 找到或自建类似I Wanna Maker的游戏生成器，支持LLM调用进行游戏内容生成，且优化LLM生成趣味性游戏内容的能力。可以先集中在生成奇观供探索，维护一个简单的核心玩法 (比如获取持续探索所需的资源)。生成奇观可能通过用户调研反馈获得性能提升。

2. 类似的，LLM可能可以生成肉鸽流派。以类吸血鬼幸存者为例，只维持简单的核心玩法。此时LLM需要生成轻度玩法，需要掌握生成趣味性轻度玩法的能力。同样可能可以通过用户调研反馈获得性能提升。